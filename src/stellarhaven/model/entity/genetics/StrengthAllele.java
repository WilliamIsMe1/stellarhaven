    ...
    public class StrengthAllele extends Allele<Integer> {
        ...
        public void mutate() {
            ...
            if (value >= 2 && value <= 9) {
                ...
                throw new IllegalStateException("StrengthAllele instances are not supposed to be above 10 or below 1");
            }
        }
        
        public static Supplier<Allele<Integer>> getSupplier() { // This returns 5 on average
            return () -> {
                int strength = (((int) (Math.random()*10) + 1) + ((int) (Math.random()*10) + 1) + ((int) (Math.random()*10) + 1)) / 3;
                return new StrengthAllele(strength);
            };
        }
    }